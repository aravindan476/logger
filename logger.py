...
def live_logs_connected():
	...
	if(len(deviceDSN)==0):
		...
		print("You have connected some devices.Pls select the one to take logs..")
...
def take_live_logs(i):
	file_name=get_filename(i)
	print(file_name)
	os.system("adb -s "+deviceDSN[i]+" logcat -v threadtime>live_"+file_name+".txt")
